...
# Página a la que vuelve el botón "Volver" (ruta vista desde la raíz del proyecto)
BACK_LINK = "index.php"
...
# Si necesitas prefijo adicional desde la raíz (ej: "assets/"), ponlo aquí:
IMG_PREFIX = ""
...
def outdir_depth(out_dir: str) -> int:
    """
    Calcula cuántos niveles de carpeta tiene OUT_DIR (si es relativo).
    Ej:
      "out_pages" -> 1  => prefix "../"
      "public/out_pages" -> 2 => prefix "../../"
      "." o "" -> 0
    """
    p = Path(out_dir)
...
def rel_from_outdir_to_root(url: str, prefix_to_root: str) -> str:
    """
    Convierte una URL/ruta pensada desde la raíz del proyecto a la ruta correcta
    vista desde OUT_DIR (ej: out_pages/).

    - Si la url es absoluta (http, https, /, #, mailto, tel) NO toca.
    - Si es relativa, le antepone prefix_to_root (ej: "../").
    """
    u = (url or "").strip()
    if not u:
        return u
...
def build_product_page(p: dict, prefix_to_root: str) -> str:
    nombre = p.get("nombre", "Producto")
    ...
    # ✅ Imagen: desde out_pages/ necesita "../"
    imagen_raw = (IMG_PREFIX + (p.get("imagen") or "")).strip()
    imagen = rel_from_outdir_to_root(imagen_raw, prefix_to_root) if imagen_raw else ""

    # ✅ Links nav / back: desde out_pages/ necesita "../"
    back_link = rel_from_outdir_to_root(BACK_LINK, prefix_to_root)
    nav_html = "".join([
        f"<li><a href='{h(rel_from_outdir_to_root(url, prefix_to_root))}'>{h(text)}</a></li>"
    ...
    img_block = f"<img src='{h(imagen)}' alt='{h(nombre)}'>" if imagen else "<img src='' alt='' style='display:none;'>"
...
def main():
    ...
    depth = outdir_depth(OUT_DIR)
    prefix_to_root = "../" * depth  # out_pages -> "../"
    ...
    for p in products:
        ...
        html_page = build_product_page(p, prefix_to_root)
